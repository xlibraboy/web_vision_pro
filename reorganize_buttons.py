#!/usr/bin/env python3
"""
Reorganize playback buttons in AnalysisView.cpp to standard media player layout.
"""
...
# Define the new playback toolbar code
new_toolbar = '''    // === PLAYBACK CONTROL TOOLBAR (Standard Media Player Layout) ===
    ...
    beginButton_->setStyleSheet(
        "QPushButton { background: #555; color: white; border-radius: 3px; font-weight: bold; font-size: 12px; }"
    ...
    playPauseButton_->setFixedSize(36, 24);
    playPauseButton_->setToolTip("Play/Pause");
    playPauseButton_->setStyleSheet(
        "QPushButton { background: #4CAF50; color: white; border-radius: 3px; font-size: 14px; font-weight: bold; }"
    ...
    resetButton_ = new QPushButton("⊙", playbackPanel_);
    resetButton_->setFixedSize(28, 24);
    resetButton_->setToolTip("Jump to Trigger Point (Frame 0)");
    resetButton_->setStyleSheet(
        "QPushButton { background: #FF5722; color: white; border-radius: 3px; font-weight: bold; font-size: 14px; }"
    ...
    connect(saveAviButton_, &QPushButton::clicked, this, &AnalysisView::onSaveAviClicked);
    toolbarLayout->addWidget(saveAviButton_);'''

# Find and replace the section between "// Speed menu button" and the last addWidget for saveAviButton_
pattern = r'(    // Speed menu button.*?toolbarLayout->addWidget\(saveAviButton_\);)'
replacement = new_toolbar
...
print("✓ Playback buttons reorganized successfully!")
