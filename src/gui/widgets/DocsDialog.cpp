...
DocsDialog::DocsDialog(QWidget* parent)
    ...
    addSection(QStringLiteral("How the App Works"), sectionHtml(
        ...
        QStringLiteral(
            ...
            "seconds to minutes to lock; the app does not yet show the lock status, so an "
            "unsynced camera silently falls back to index-based mapping. With chunk data "
            "off, events use the software timestamps, which are cross-camera comparable "
            "regardless of PTP.</p>"
            "<h3>Defect sync</h3>"
