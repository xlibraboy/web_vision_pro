...
DocsDialog::DocsDialog(QWidget* parent)
    ...
    addSection(QStringLiteral("How the App Works"), sectionHtml(
        ...
        QStringLiteral(
            ...
            "<li>Each trigger row selects the machine sections it <b>Records</b> (Wire, "
            "Press-Part, Pre-Dryer, After-Dryer, Calender-Reel). All sections = every active "
            "camera; a narrower selection records only cameras assigned to those sections, so a "
            "Press-Part break can be set to leave the downstream sections out. The trigger's "
            "Group only labels the event.</li>"
            "<li>Each event stores per-camera video, per-frame <b>sensor and host timestamps</b> "
            "plus frame counters, and the machine speed + camera positions captured at trigger "
            "time.</li>"
            ...
            "<h3>Time sync (sensor &amp; host clocks, PTP)</h3>"
            "<p>Every recorded frame carries two times plus a frame counter:</p>"
            "<ul>"
            "<li><b>Sensor time</b> - the camera's own <b>hardware chunk timestamp</b>, generated "
            "by the camera when the image was taken. It requires per-camera chunk data with "
            "Timestamp + Framecounter enabled (the default). This is the true exposure time: no "
            "network or scheduling delay.</li>"
            "<li><b>Host arrival time</b> - when the frame reached this PC. It is always recorded, "
            "for every frame. It includes transfer and OS jitter (milliseconds), but it is one "
            "shared clock across all cameras.</li>"
            "</ul>"
            "<p>When chunks are off or unavailable (e.g. emulated cameras), the sensor time is "
            "filled with the host time, so single-camera playback always has a usable clock.</p>"
            "<p><b>Which clock playback uses:</b> one camera's own relative-time axis, frame steps "
            "and trigger frame use its sensor time (falling back to host time). Playback across "
            "cameras - showing the same defect on every tile - maps through the sensor clocks when "
            "they are comparable across cameras, through the shared host clock when they are not "
            "(see PTP below), and falls back to frame-index mapping only for legacy events that "
            "carry neither. Mark-based or speed alignment is applied on top in every case.</p>"
            "<p><b>PTP (IEEE 1588)</b> synchronizes the cameras' clocks to one master, which is "
            "what makes their sensor times comparable. The app enables it on every real camera "
            "that has the feature (<b>GevIEEE1588</b> on scout/acA-class models, <b>PtpEnable</b> "
            "on ace 2); a camera with neither logs that the node was not found and keeps running. "
            "Precision depends on the network: a plain switch still passes PTP but offsets can "
            "drift to milliseconds, while a transparent/boundary-clock switch keeps them in the "
            "microsecond range. Without a GPS master the epoch is arbitrary (ARB mode) - the "
            "cameras agree with each other, but the epoch can shift if the master reboots; a GNSS "
            "grandmaster makes the timestamps absolute (UTC). Cameras without PTP (e.g. the scout "
            "fleet) stamp from their own power-on counter, so cross-camera playback for those "
            "lines uses the shared host clock instead.</p>"
            "<p>The lock state is shown live: each Live View tile carries a PTP badge "
            "(<b>PTP SLAVE</b> green / <b>PTP MASTER</b> blue / <b>PTP SYNC</b> amber while the "
            "clock is still settling / <b>PTP OFF</b>; hidden when the camera has no IEEE 1588 "
            "feature), and the camera's Device Settings status card shows the state with the "
            "current offset from the master clock (hover for the clock IDs). A slave offset over "
            "<b>1 ms</b> raises a status-bar warning until it recovers.</p>"
            "<h3>Defect sync</h3>"
    ...
    addSection(QStringLiteral("Workflows"), sectionHtml(
        ...
        QStringLiteral(
            ...
            "position (mm), its group, and the sections it records (Records). Only the <b>first</b> "
            "sensor's True starts the recording - while an event records, later sensors are "
            "ignored. A sensor "
    ...
    addSection(QStringLiteral("Tutorial"), sectionHtml(
        ...
        QStringLiteral(
            ...
            "For sensor-accurate frame times across cameras, keep per-camera chunk timestamps on "
            "(the default) and make sure PTP locks - all cameras on a PTP-capable network then "
            "share one clock. Cameras without PTP still align through the shared host arrival "
            "clock, so cross-camera review keeps working; watch the PTP badge on each Live View "
            "tile (or the PTP row in the camera's Device Settings card) for the lock state, and "
            "treat a status-bar warning about a slave camera's offset over 1 ms as a network/PTP "
            "issue to fix.</p>"
            "<h3>10. Handy tips</h3>"
            "<ul>"
