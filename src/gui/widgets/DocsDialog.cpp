...
DocsDialog::DocsDialog(QWidget* parent)
    ...
    addSection(QStringLiteral("How the App Works"), sectionHtml(
        ...
        QStringLiteral(
            ...
            "<p>Every recorded frame stores a nanosecond timestamp, a frame counter and the "
            "<b>host arrival time</b>. The timestamp comes from the camera's <b>hardware chunk "
            "timestamp</b> (true sensor readout time; requires per-camera chunk data with "
            "Timestamp + Framecounter enabled) and, when chunks are off or unavailable (e.g. "
            "emulation), from when the frame reaches this PC. The host arrival time is always "
            "stored alongside it.</p>"
            "<p>Basler GigE cameras synchronize their clocks with <b>PTP</b> (IEEE 1588); "
            "the app enables it on every real camera that has the feature. Once PTP locks, all "
            ...
            "seconds to minutes to lock. Cameras without PTP (e.g. the scout fleet) stamp "
            "from their own power-on counter, so playback aligns those cameras through the "
            "shared host arrival clock instead.</p>"
            ...
            "now visible instead of silent. Aligned playback needs cross-comparable times: "
            "it uses the sensor clocks when they agree, the host clock when they do not, "
            "and index-based mapping only for legacy events that carry neither.</p>"
            "<h3>Defect sync</h3>"
