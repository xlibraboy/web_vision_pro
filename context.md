...
- Every saved event frame carries a per-frame **timestamp (ns) + frame counter + host arrival time** (`FrameMetadata` in `RawFormat.h`, format v2; v1 files have host = 0). Two sources feed the camera stamp:
  - **Hardware chunk timestamp** (CameraManager grab path): `ChunkTimestamp` ticks converted to ns via `GevTimestampTickFrequency` (125 MHz / 8 ns fallback for scout-class). Requires per-camera chunk mode on (`chunkModeActive` + `enabledChunks` containing `Timestamp`/`Framecounter`); default is now **on** with a one-time migration for saved configs.
  - **Software fallback** (emulation, or chunk mode off/invalid): host `system_clock` at frame arrival.
  - The **host arrival time is always recorded** (both paths, v2 files), so review can still align cameras whose hardware clocks are not comparable.
- **PTP (IEEE 1588-2008)**: `CameraManager` enables it on every real (non-emulated) camera at config apply, writing `GevIEEE1588` (scout/acA-class) or `PtpEnable` (ace 2/dart M), whichever the camera exposes; cameras with neither log `PTP enable node not found/writable`. PTP slaves each camera's clock — and therefore its chunk timestamps — to the best master clock on the network, making hardware timestamps comparable across cameras at the same real instant. Precision depends on the network (PTP-aware switches; a GPS master only for absolute UTC/TAI time, otherwise the domain runs in arbitrary-timescale mode and one camera becomes grandmaster).
- **Current fleet is PTP-less**: the scout scA780-class cameras expose no IEEE 1588 nodes (the local scout manual documents none), so chunk stamps are **camera-local counters** (epoch = camera power-on) and the app falls back to the recorded host arrival clock for cross-camera mapping.
- **PTP state readout** (`CameraManager::readPtpStatus` / `fillPtpStatusFromNodeMap`, sampled every 10 s by the temperature monitor thread, mutex-serialized against grab-thread GenApi access): fires the PTP latch command (`GevIEEE1588DataSetLatch` or `PtpDataSetLatch`) so the values form one snapshot, then reads `GevIEEE1588Status` (Slave/Master/Initializing/…), `GevIEEE1588OffsetFromMaster` (ticks → ns via `GevTimestampTickFrequency`), `PtpServoStatus=Locked` where available, and the clock/parent-clock IDs. Pushed through a `PtpStatusCallback` → `MainWindow` → `LiveDashboard`:
  - **Live View tile badge** (bottom-left): `PTP SLAVE` (green) / `PTP MASTER` (blue) / `PTP SYNC` (amber, clock still settling) / `PTP OFF` / `PTP FAULT`; hidden on cameras without IEEE 1588 nodes (emulation or a PTP-less fleet).
  - **Device Settings dialog status card** (refreshed every 2 s via `LiveDeviceSettings.ptp`): `PTP  Slave · <offset>` (green, e.g. µs/ms), `Master` (blue), `Syncing…` (amber), `Off`, or `—`; hover shows the camera's clock ID and the grandmaster clock ID.
  - **Offset threshold alert** (`MainWindow`): when a slave's offset exceeds 1 ms (`kMaxPtpOffsetFromMasterNs`, hard-coded), a status-bar warning fires on the rising edge only (per-camera `ptpOffsetAlertCameras_` set) — `🟡 PTP OFFSET HIGH: Camera N is X ms from the master clock…` — plus a short “back to normal” note when it recovers. Masters, PTP-off, and mid-sync cameras are ignored.
- **Consumers**: `AnalysisView` caches every camera's per-frame sensor stamps *and* host arrival stamps on event load and maps the timeline camera's frame time to each other camera's *nearest own frame* for mixed-fps playback (`displayedFrameIndexForCamera`, `tlIndexOfOwnFrame`). The sensor clock is used when the cameras agreed at event start (< 60 s guard per series); otherwise the **host clock** series is tried (one shared PC clock, always comparable), and only when both are unusable does it fall back to shared-index mapping.
